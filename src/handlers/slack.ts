...
import { getGleapClient, getGleapTicketUrl, type GleapTicket } from "../integrations/gleap/client"
...
const verifySlackSignature = (signature: string, timestamp: string, rawBody: string): boolean => {
...
const updateThreadStatus = async (ctx: SlackActionContext, statusText: string) => {
  const slack = getSlackClient()
...
const handleReject = async (gleapTicketId: string, reason: string, ctx?: SlackActionContext) => {
  const gleap = getGleapClient()
  ...
  if (ctx) {
    await Promise.all([
      ...
      postActionMessage(
        ctx,
        `❌ Rejected by <@${ctx.userId}> ${reason} <${getGleapTicketUrl(gleapTicketId, ticket.type)}|Open in Gleap>`,
      ),
    ])
  }
}
...
const openConfirmModal = async (
      ...
      ? [
          {
            ...
            element: {
              ...
              options: trackerTickets.slice(0, 100).map((t) => ({
                text: { type: "plain_text" as const, text: `#${t.bugId} ${t.title}`.slice(0, 75) },
                value: t.id,
              })),
            },
          },
  ...
  if (!res.ok) console.error(`[Slack] views.open (confirm) failed: ${res.error}`)
}
...
const openRejectModal = async (
  ...
  if (!res.ok) console.error(`[Slack] views.open (reject) failed: ${res.error}`)
}
...
export async function slackPost(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (payload.type === "block_actions") {
      ...
      if (action?.action_id === "confirm") {
        ...
        const [ticket, { tickets: trackerTickets }] = await Promise.all([
          gleap.tickets.get(gleapTicketId),
          gleap.tickets.list({ type: config.gleap.trackerTicketType, trackerTicket: true }),
        ])
    ...
    if (payload.type === "view_submission") {
      ...
      const values = (view.state as Record<string, unknown>)?.values as Record<string, unknown>
      ...
      if (view.callback_id === "confirm_modal") {
        ...
        const customTitle = (
          (values?.title_block as Record<string, unknown>)?.title_input as Record<string, unknown>
        )?.value as string | undefined

        const description = (
          (values?.desc_block as Record<string, unknown>)?.desc_input as Record<string, unknown>
        )?.value as string | undefined
      ...
      if (view.callback_id === "reject_modal") {
        const reason =
          ((
            (values?.reason_block as Record<string, unknown>)?.reason_input as Record<
              string,
              unknown
            >
          )?.value as string) ?? ""
