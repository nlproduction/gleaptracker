  ...
  const updatedBlocks = existingBlocks
    ...
    .map((b) => {
      ...
      const kept = elements.filter(
        (e) => !["confirm", "reject"].includes(e.action_id),
      );
...
const handleConfirm = async (gleapTicketId, opts = {}, ctx) => {
  ...
  if (ctx) {
    await Promise.all([
      ...
      postActionMessage(
        ctx,
        `⏳ Confirmed, will be fixed soon. <${(0, client_1.getGleapTicketUrl)(gleapTicketId, ticket.type)}|Open in Gleap>`,
      ),
    ]);
  }
};
const handleReject = async (gleapTicketId, reason, ctx) => {
  ...
  if (ctx) {
    await Promise.all([
      ...
      postActionMessage(
        ctx,
        `❌ Rejected: ${reason} <${(0, client_1.getGleapTicketUrl)(gleapTicketId, ticket.type)}|Open in Gleap>`,
      ),
    ]);
  }
};
      ...
      if (
        ...
        if (text.includes("g:note")) {
          ...
          if (content) {
            ...
            if (gleapTicketId) {
              await (0, client_1.getGleapClient)().messages.addNote(
                gleapTicketId,
                content,
              );
    ...
    if (payload.type === "view_submission") {
      ...
      if (view.callback_id === "confirm_modal") {
        const existingTrackerId =
          values?.existing_tracker_block?.tracker_select?.selected_option
            ?.value;
